...
for(i in 1:nrow(grid)) {
  ...
  for (k in 1:length(cvIndex)){
    ...
    traincv_data <- traincv_data[,colnames(traincv_data) %in% 
                                   c(num_covariates, cat_covariates, y)]
...
ggplot(var_imp, 
  ...
  xlab("")+
  ggtitle("RF Variable Importance (Impurity)")+
  guides(fill=F)+
...
ols_1 <- update(ols, . ~ . - year_last_pub)
ols_2 <- update(ols_1, . ~ . - count_unique_coauthor)
ols_3 <- update(ols_2, . ~ . - pred_male)

pred_ols <- predict(ols_3, test_data)
...
pred_all <- ggplot() +
  ...
  labs(y = "2021 Pay", x = "All Faculty", title = "Predictions on Test Set")
